...
// that client stopped, which is the client's own problem -- a store pod
// restarted, was evicted or lost its node -- and the master must act on it at
// once. A single-client cell is protected by the hold condition instead, which
...
class MasterService {
   ...
   private:
    ...
    // Capacity a standby restore is still accounting for, per segment name.
    // Only HA snapshot restore puts an entry here, so a non-HA master's map is
    // always empty -- which is what lets ReMountSegment skip its shard walk.
