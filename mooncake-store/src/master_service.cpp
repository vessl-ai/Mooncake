...
auto MasterService::ReMountSegment(const std::vector<Segment>& segments,
    ...
    -> tl::expected<void, ErrorCode> {
    {
        ...
        // The shard walk below only has replicas to fix up for a segment
        // whose standby-restored replicas are still accounted, which is a
        // state only HA snapshot restore produces. Without one, the walk is
        // the whole master's cost -- it holds client_mutex_ and
        // snapshot_mutex_ exclusively for as long as the metadata is large --
        // to produce nothing.
...
void MasterService::ClientMonitorFunc() {
    ...
    // mass expiry is one bad tick away from erasing its cell's whole cache.
    ...
    while (client_monitor_running_) {
        ...
        // with no snapshot and no HA that is unrecoverable: one bad tick costs
        // a cell its whole L3 cache. So before acting, the monitor has to tell
        // "my clients died" from "I failed to process their heartbeats" -- and
        ...
        // every store pod restart in a single-client cell for the whole grace.
        //
        ...
        // other way costs the cell its whole cache.
        //
        // What the rule does, case by case:
        //
        //   1-client cell, that client dies, master healthy      expire now
        ...
        //   13 clients, healthy master, the whole fleet dies     defer, then
        //                                                        expire at the
        //                                                        grace bound
        ...
        //   1-client cell, starvation with no client_mutex_ hold  keys erased
